	...
	do {
		...
		if(mask & PLAYING_MASK){
			...
			if(to_write <= 0){
				...
				err = rt_event_signal(&evenement, END_FILE_MASK | DISPLAY_RESET_MASK);
				if (err != 0) {
					printf("rt_event_signal failed: %s\n", strerror(-err));
					return;
				}
		...
		if(rt_task_wait_period(NULL)){
			//rt_printf("rt_wait_period fail [%s]\n", __FUNCTION__);
		}
...
void sdcard_writer_task(void *cookie)
{
	...
	do {
		...
		if (mask & NEW_RECORDING_MASK) {
			...
			if(write_wav_header(args->audio_fd, &wh)){
				rt_printf("write_wav_header failed [%s]\n", __FUNCTION__ );
				exit(EXIT_FAILURE);
			}
...
void processing_task(void *cookie)
{
	...
	if(sample_volume == NULL){
		rt_printf("Error malloc [%s]\n", __FUNCTION__ );
		exit(EXIT_FAILURE);
	}
	...
	do {
		...
		if (is_pressed_now(old_key_val, key_val, KEY0) ){
			if(mode == RECORDING){
				...
				mode = PAUSED;
				err = rt_event_clear(&evenement, RECORDING_MASK, NULL);
				if (err != 0) {
					printf("rt_event_clear failed: %s\n", strerror(-err));
					return;
				}
				err = rt_event_signal(&evenement, PAUSED_MASK | END_RECORDING_MASK | DISPLAY_RESET_MASK);
				if (err != 0) {
					printf("rt_event_signal failed: %s\n", strerror(-err));
					return;
				}
				...
				rt_printf("Recording...\n");
				err = rt_event_clear(&evenement, PAUSED_MASK  | PLAYING_MASK, NULL);
				if (err != 0) {
					printf("rt_event_clear failed: %s\n", strerror(-err));
					return;
				}
				err = rt_event_signal(&evenement, RECORDING_MASK | NEW_RECORDING_MASK | DISPLAY_RESET_MASK);
				if (err != 0) {
					printf("rt_event_signal failed: %s\n", strerror(-err));
					return;
				}
		...
		if (is_pressed_now(old_key_val, key_val, KEY1) ){
			if(mode != RECORDING){
				...
				if(mask & END_FILE_MASK){
					...
					mode = PLAYING;
					err = rt_event_clear(&evenement, PAUSED_MASK, NULL);
					if (err != 0) {
						printf("rt_event_clear failed: %s\n", strerror(-err));
						return;
					}
					err = rt_event_signal(&evenement, PLAYING_MASK);
					if (err != 0) {
						printf("rt_event_signal failed: %s\n", strerror(-err));
						return;
					}
					...
					err = rt_event_signal(&evenement, PAUSED_MASK);
					if (err != 0) {
						printf("rt_event_signal failed: %s\n", strerror(-err));
						return;
					}
...
main(int argc, char *argv[])
{
	...
	/* Setup task barrier */
	if (rt_sem_create(&task_barrier, "Task barrier", 0, S_PRIO)) {
	...
	/* Q_UNLIMITED car on ne se préoccupe pas du nombre de messages max dans la boîte aux lettres */
	err = rt_queue_create(&audio_data_in, "audio_data_in", QUEUE_SIZE, Q_UNLIMITED, Q_PRIO);
	if (err != 0) {
		printf("rt_queue_create error [%s]\n", __FUNCTION__ );
		exit(EXIT_FAILURE);
	}
	err = rt_queue_create(&sd_out, "sd_out", QUEUE_SIZE, Q_UNLIMITED, Q_PRIO);
	if (err != 0) {
		printf("rt_queue_create error [%s]\n", __FUNCTION__ );
		exit(EXIT_FAILURE);
	}
	err = rt_queue_create(&audio_out, "audio_out", QUEUE_SIZE, Q_UNLIMITED, Q_PRIO);
	if (err != 0) {
		printf("rt_queue_create error [%s]\n", __FUNCTION__ );
		exit(EXIT_FAILURE);
	}
	...
	err = rt_task_spawn(&source_rt_task, SOURCE_TASK_NAME, STACK_SIZE, PRIO_TASKS_AUDIO, T_JOINABLE, source_task, &args);
	if (err != 0) {
		printf("rt_task_spawn error [%s]\n", __FUNCTION__ );
		exit(EXIT_FAILURE);
	}

	err = rt_task_spawn(&time_display_rt_task, TIME_DISPLAY_TASK_NAME, STACK_SIZE, PRIO_TASKS_DISPLAY, T_JOINABLE, time_display_task, NULL);
	if (err != 0) {
		printf("rt_task_spawn error [%s]\n", __FUNCTION__ );
		exit(EXIT_FAILURE);
	}

	err = rt_task_spawn(&control_rt_task, CONTROL_TASK_NAME, STACK_SIZE, PRIO_TASKS_CONTROL, T_JOINABLE, control_task, NULL);
	if (err != 0) {
		printf("rt_task_spawn error [%s]\n", __FUNCTION__ );
		exit(EXIT_FAILURE);
	}

	err = rt_task_spawn(&audio_output_rt_task, AUDIO_OUTPUT_TASK_NAME, STACK_SIZE, PRIO_TASKS_AUDIO, T_JOINABLE, audio_output_task, NULL);
	if (err != 0) {
		printf("rt_task_spawn error [%s]\n", __FUNCTION__ );
		exit(EXIT_FAILURE);
	}

	err = rt_task_spawn(&processing_rt_task, PROCESSING_TASK_NAME, STACK_SIZE, PRIO_TASKS_AUDIO, T_JOINABLE, processing_task, NULL);
	if (err != 0) {
		printf("rt_task_spawn error [%s]\n", __FUNCTION__ );
		exit(EXIT_FAILURE);
	}

	err = rt_task_spawn(&sdcard_writer_rt_task, SDCARD_WRITER_TASK_NAME, STACK_SIZE, PRIO_TASKS_AUDIO, T_JOINABLE, sdcard_writer_task, &args);
	if (err != 0) {
		printf("rt_task_spawn error [%s]\n", __FUNCTION__ );
		exit(EXIT_FAILURE);
	}
